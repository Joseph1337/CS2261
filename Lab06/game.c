...
void initGame() {
    ...
    //TODO 4.2: Load the katamari image's colors
	DMANow(3, katamariPal, PALETTE, 256);
...
void initBalls() {
	...
	for (int i = 0; i < BALLCOUNT; i++) {
		...
		balls[i].cdel = 2;
		balls[i].active = 1;
		balls[i].isKatamari = 0;

		// Make the first ball a katamari
		if(i == 0){
			balls[i].isKatamari = 1;
			balls[i].height = 16;
			balls[i].width = 16;
		} else {
			balls[i].height = 10;
...
void updateBall(BALL* b) {
	...
	if (b->active) {
		...
		for (int i = 0; i < BULLETCOUNT; i++) {
			if (bullets[i].active && !b->isKatamari && collision(b->col, b->row, b->width, b->height,
...
void drawBall(BALL* b) {
	...
	if(b->active) {
		if (b->isKatamari)
			drawImage4(b->col, b->row, b->width, b->height, katamariBitmap);
		else
